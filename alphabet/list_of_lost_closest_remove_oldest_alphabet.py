...
class ListOfLostAlphabet(symbol_alphabet.SymbolAlphabet):
    ...
    def _add(self, normalized, symbol_shift ,label=None):
        ...
        self.counters[new_symbol] = (1, self.tick)
        self.tick += 1
    ...
    def get_similar(self, normalized, symbol_shift, label=None):
        ...
        if similar_symbol:
            self.counters[similar_symbol] = (self.counters[similar_symbol][0] + 1, self.tick)
            self.tick += 1
            ...
            for key_symbol in self.counters.keys(): # no symbols are removed, they are only decremented
                self.counters[key_symbol] = (self.counters[key_symbol][0] - 1, self.counters[key_symbol][1])
            ...
            # least_frequent is a list of tuples (key, value)
            if(len(least_frequent) > 0 and least_frequent[0][1][0] <= 0):
