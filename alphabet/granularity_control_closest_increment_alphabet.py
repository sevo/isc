...
class GranularityAlphabet():
    ...
    def get_similar(self, normalized, symbol_shift, label=None):
        ...
        if similar_symbol:
            ...
            return (self._add(normalized, symbol_shift, label), None, float('inf'), [])
        else:
...
class GranularityControlClosestIncrementAlphabet(symbol_alphabet.SymbolAlphabet):
    ...
    def get_similar(self, normalized, symbol_shift, label=None):
        ...
        for alphabet in self.alphabets:
            similar_symbol, closest_symbol, dist, removed = alphabet.get_similar(normalized, symbol_shift, label)
            if old_removed:
        ...
        if closest: # teraz uz je jasne, ze sa bude vracat closest a ze sa decrementoval. Ak ho chcem vo finale inkrementovat o 1, tak ho musim inkrementovat o 2. Kedze sa mohol presuvat medzi abecedami, tak ho musim najskor najst
            for alphabet in self.alphabets:
                if closest in alphabet.symbols():
                    alphabet.counters[closest] = alphabet.counters[closest] + 2
        return closest
    ...
    def get_closest(self, normalized, symbol_shift=0):
        try:
            return min(filter(lambda symbol: symbol.symbol_shift == symbol_shift, chain(*[alphabet.counters.keys() for alphabet in self.alphabets])), 
            key=lambda symbol: self.distance_operator.distance(symbol.series, series))
        except ValueError:
            return None